...
while getopts "m:w:" opt; do
    case $opt in
        m) md="$OPTARG" ;;
        w) w3mopt="$OPTARG" ;;
        *) echo "Usage: $0 [-m html|text|both] [-w 'w3m_options'] <urls_list.txt> <output_dir>" >&2; exit 1 ;;
    esac
done
...
if [[ ! "$md" =~ ^(html|text|both)$ ]] || [[ ! -f "$urls" ]] || [[ -z "$odir" ]]; then
    echo "Usage: $0 [-m html|text|both] [-w 'w3m_options'] <urls_list.txt> <output_dir>" >&2
    exit 1
fi
