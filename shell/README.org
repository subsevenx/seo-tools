...
** Crawl.sh
A basic HTML crawler built on top of =w3m= that takes a text file comprising URLs and crawls them for content. Supports custom headers  and features three modes of crawl: text, html, or both.
...
By default, the crawler passes the following flags: =-dump= (text) and/or =-dump_source= (HTML).
